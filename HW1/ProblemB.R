...
ALOHAnetwork <- function() {
  ...
  for (i in 1:sample_size){
    ...
    if (transmission_attempts != 2){
      ...
      if(a & random_send()){
        a <- FALSE# function to generate random probablity to send message
        random_send <- function() sample(c(0,1), size = 1, replace = TRUE, prob = c(0.4, 0.6))
        ...
        ALOHAnetwork <- function() {
          ...
          for (i in 1:sample_size){
            ...
            if (transmission_attempts != 2){
              ...
              # sicne epoch 2 in only reachable in the code if at most 1 transmission attempt was made during
